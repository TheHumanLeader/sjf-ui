...
- `height` 接受数字（按 px）或 CSS 长度，并定义 **List 可视窗口** 的高度。
- 设置 `height` 后，分页按钮悬浮在窗口边缘，不再额外占一整行 / 一整列；不可继续翻页的一侧自动隐藏。
- 竖向 List 使用上 / 下悬浮按钮，横向 List 使用左 / 右悬浮按钮；横向内容会为边缘按钮预留少量空间。
- 分页按钮每次平滑移动一个当前可视区域；滚轮仍可连续缓动，到达边界后允许外层页面继续滚动。
